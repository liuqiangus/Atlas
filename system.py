...
class SYS:
    ...
    def action_to_conf(self, action):
        ...
        an_slice_conf.an_rb_ul       = int(np.clip(action.bandwidth_ul, 6, 43)) # RB  in UL XXX
        an_slice_conf.an_sche_alg_ul = 0 # only one ul alg
        an_slice_conf.an_maxmcs_ul   = int(np.clip(action.mcs_offset_ul, 0, 10)) #
        an_slice_conf.an_rb_dl       = int(np.clip(action.bandwidth_dl/3, 1, 14)) # RBG in DL XXX 
        an_slice_conf.an_sche_alg_dl = 0
        an_slice_conf.an_maxmcs_dl   = int(np.clip(action.mcs_offset_dl, 0, 10)) # attention XXX here mcs is minus
        tn_slice_conf.tn_bw          = int(np.clip(action.backhaul_bw, 3, 93))
        cp_slice_conf.cp_cpu         = float(np.clip(int(action.cpu_ratio*100)/100, 0.3, 1.3))
    ...
    def step(self, action):
        ...
        """
        self.enforce_idx(action)

        # this two line codes is to eliminate the previous affects, e.g., unfinished queuing tasks
        self.retrieve_indiv_performance()

        # this is for real performance collection
        time.sleep(TIME_INTERVAL)
        performance, queuesz, computetime, reliability_ul, reliability_dl = self.retrieve_indiv_performance()

        # return state, performance, False, reliability
        return np.array(performance), reliability_ul, reliability_dl
    
    def grid_search(self, threshold, traffic=1):
        times = 4 # XXX TODO data collection in offline is 5
        # this is for grid searching under different 
        VARIABLES = [   {   "bandwidth_ul": np.arange(0.1, 40, 40/times), \
        ...
        PBOUNDS = VARIABLES[args.idx]
        grid = ParameterGrid(PBOUNDS)
        print("grid search length:", len(grid))

        saver = JSONSaver(path="online_learning_dataset_numUEs_"+str(traffic)+"_grid_search.json")
        from tqdm import tqdm

        for idx in tqdm(range(len(saver._data), len(grid))):
            ...
            action_dict = resource.to_dict() # convert to dict as it is the input of BNN/GP here, rather than an action object
            ...
            qoe, perf = calculate_qoe_mar(performance, threshold)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threshold', type=float, default=300) #  for mar slice latency 
    ...
    sys = SYS(idx=args.idx, traffic=args.traffic, num_vars=6) # create the sys class
    sys.reset() # inform traffic to users here
    ################################################################################################################
    ################################################################################################################
    ################################################################################################################
    if args.mode == "indiv":
        ...
        performance, queuesz, computetime, reliability_ul, reliability_dl = sys.retrieve_indiv_performance()
        for _ in tqdm(range(120)):time.sleep(1)
        performance, queuesz, computetime, reliability_ul, reliability_dl = sys.retrieve_indiv_performance()
    ...
    ################################################################################################################
    if args.mode == "grid":
        sys.grid_search(threshold=args.threshold, traffic=args.traffic)
