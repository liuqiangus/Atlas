...
def main():
    ...
    inputs = np.random.random((1000, 14)).astype(np.float32)
    outputs = np.sum(inputs, axis=-1).astype(np.float32)
    ...
    if args.mode == 'train':
        for epoch in range(1, args.epochs + 1):
            ...
            torch.save(model.state_dict(), args.save_dir + "/mnist_sfc.pth")
    elif args.mode == 'test':
        checkpoint = args.save_dir + '/mnist_sfc.pth'
